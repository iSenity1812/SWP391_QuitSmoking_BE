...
public class QuitPlanDatesValidator implements ConstraintValidator<ValidQuitPlanDates, QuitPlan> {
    ...
    public boolean isValid(QuitPlan quitPlan, ConstraintValidatorContext context) {
        ...
        LocalDate createDate = quitPlan.getCreateDate();
        ...
        // if (createdAt != null &&
        // createdAt.isAfter(currentTime.plusSeconds(TIMING_TOLERANCE_SECONDS))) {
        // context.buildConstraintViolationWithTemplate("Ngày tạo kế hoạch không thể ở
        // tương lai")
        ...
        // Nếu goalDate chưa bằng hay vượt qua thời gian thực thì status không thể là
        // FAILED hay COMPLETED
        if (goalDate != null && currentTime.toLocalDate().isBefore(goalDate)) {
            if (status == QuitPlanStatus.FAILED || status == QuitPlanStatus.COMPLETED) {
                context.buildConstraintViolationWithTemplate(
                        "Kế hoạch không thể ở trạng thái 'Hoàn thành', 'Thất bại' khi ngày mục tiêu còn ở tương lai")
        ...
        if (goalDate != null && currentTime.toLocalDate().isAfter(goalDate)) {
            if (status != QuitPlanStatus.FAILED && status != QuitPlanStatus.COMPLETED) {
                context.buildConstraintViolationWithTemplate(
                        "Kế hoạch phải ở trạng thái 'Hoàn thành', 'Thất bại' khi ngày mục tiêu đã qua")
                        .addPropertyNode("status")
                        .addConstraintViolation();
                isValid = false;
            }
        ...
        // Nếu thời gian thực chưa bằng hay vượt qua startdate thì status phải là
        // NOT_STARTED
        if (startDate != null && futureToleranceTime.isBefore(startDate.atStartOfDay())) {
            if (status == QuitPlanStatus.COMPLETED || status == QuitPlanStatus.IN_PROGRESS ||
                    status == QuitPlanStatus.FAILED) {
                context.buildConstraintViolationWithTemplate(
                        "Kế hoạch không thể ở trạng thái 'Hoàn thành', 'Đang tiến hành', 'Thất bại', hoặc 'Bị bỏ dở' khi ngày bắt đầu còn ở tương lai")
                        .addPropertyNode("status")
                        .addConstraintViolation();
                isValid = false;
            }
        ...
        // Nếu thời gian thực đã bằng hay vượt qua startdate thì status không thể
        // NOT_STARTED
        if (startDate != null && startDate.atStartOfDay().isAfter(futureToleranceTime)) {
            if (status != QuitPlanStatus.NOT_STARTED) {
                context.buildConstraintViolationWithTemplate(
                        "Kế hoạch phải ở trạng thái 'Chưa bắt đầu' khi ngày bắt đầu còn ở tương lai")
                        .addPropertyNode("status")
                        .addConstraintViolation();
                isValid = false;
            }
        ...
        if (startDate != null && startDate.atStartOfDay().isBefore(tolerantCurrentTime)) {
            if (status == QuitPlanStatus.NOT_STARTED) {
                context.buildConstraintViolationWithTemplate(
                        "Kế hoạch không thể ở trạng thái 'Chưa bắt đầu' khi ngày bắt đầu đã qua")
                        .addPropertyNode("status")
                        .addConstraintViolation();
                isValid = false;
            }
        ...
        // Nếu Status là IN_PROGRESS, thì GoalDate phải ở hiện tại hoặc tương lai, và
        // StartDate phải ở quá khứ hoặc hiện tại
        if (status == QuitPlanStatus.IN_PROGRESS) {
            if (goalDate != null) {
                if (currentTime.toLocalDate().isAfter(goalDate)) { // currentTime > goalDate
                    context.buildConstraintViolationWithTemplate(
                            "Ngày mục tiêu phải ở hiện tại hoặc tương lai khi kế hoạch đang 'Đang tiến hành'")
                ...
                context.buildConstraintViolationWithTemplate(
                        "Ngày mục tiêu không được để trống khi kế hoạch đang 'Đang tiến hành'")
                        .addPropertyNode("goalDate")
                        .addConstraintViolation();
                isValid = false;
            }
            ...
            if (startDate != null) {
                if (futureToleranceTime.isBefore(startDate.atStartOfDay())) { // currentTime < startDate
                    context.buildConstraintViolationWithTemplate(
                            "Ngày bắt đầu phải ở quá khứ hoặc hiện tại khi kế hoạch đang 'Đang tiến hành'")
                            .addPropertyNode("startDate")
                            .addConstraintViolation();
                    isValid = false;
                }
                ...
                context.buildConstraintViolationWithTemplate(
                        "Ngày bắt đầu không được để trống khi kế hoạch đang 'Đang tiến hành'")
                        .addPropertyNode("startDate")
                        .addConstraintViolation();
                isValid = false;
            }
