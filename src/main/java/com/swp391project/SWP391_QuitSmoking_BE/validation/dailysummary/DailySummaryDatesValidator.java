...
public class DailySummaryDatesValidator implements ConstraintValidator<ValidDailySummaryDates, DailySummary> {
    ...
    public boolean isValid(DailySummary dailySummary, ConstraintValidatorContext context) {
        ...
        LocalDate quitPlanStartDate = quitPlan.getStartDate();
        ...
        if (quitPlanStartDate != null && quitPlanGoalDate != null) {
            if (trackDate.isBefore(quitPlanStartDate) || trackDate.isAfter(quitPlanGoalDate)) {
                context.buildConstraintViolationWithTemplate(
                        "Ngày theo dõi phải nằm trong khoảng thời gian của kế hoạch cai thuốc (từ ngày bắt đầu đến ngày mục tiêu)")
            ...
            context.buildConstraintViolationWithTemplate(
                    "Thông tin kế hoạch (StartDate, GoalDate) không đầy đủ để kiểm tra ngày theo dõi")
                    .addPropertyNode("trackDate")
                    .addConstraintViolation();
            isValid = false;
        }
        ...
        if (existingSummary.isPresent() && (dailySummary.getDailySummaryId() == null
                || !existingSummary.get().getDailySummaryId().equals(dailySummary.getDailySummaryId()))) {
        ...
        // điều kiện để goal trong ngày đó chỉ được true khi smokedCount <= số điếu nên
        // hút
        if (dailySummary.isGoalAchievedToday()) { // Nếu đánh dấu là kế hoạch đã hoàn thành trong ngày
            if (quitPlanStartDate != null && quitPlanGoalDate != null) {
                // ChronoUnit.DAYS.between: tính số ngày giữa hai LocalDate + 1 để bao gồm cả
                // ngày bắt đầu
                long totalDays = ChronoUnit.DAYS.between(quitPlanStartDate, quitPlanGoalDate) + 1;
                ...
                if (reductionType != null) {
                    ...
                    long dayIndex = ChronoUnit.DAYS.between(quitPlanStartDate, trackDate) + 1;
                    ...
                    if (allowedDay.isPresent()) {
                        ...
                        if (dailySummary.getTotalSmokedCount() > allowedCigarettes) {
                            context.buildConstraintViolationWithTemplate("Số điếu thuốc đã hút ("
                                    + dailySummary.getTotalSmokedCount() + ") vượt quá số điếu cho phép ("
                                    + allowedCigarettes + ") để đánh dấu 'Hoàn thành kế hoạch'")
                                    .addPropertyNode("smokedCount")
                                    .addConstraintViolation();
                            isValid = false;
                        }
                    } else {
                        // Trường hợp không tìm thấy ngày trong kế hoạch (lỗi logic hoặc ngày ngoài phạm
                        // vi)
                        context.buildConstraintViolationWithTemplate(
                                "Không thể xác định số điếu thuốc cho phép cho ngày này trong kế hoạch")
                                .addPropertyNode("trackDate")
                                .addConstraintViolation();
                        isValid = false;
                    }
                ...
                context.buildConstraintViolationWithTemplate(
                        "Thông tin kế hoạch (StartDate, GoalDate, PlanType) không đầy đủ để kiểm tra hoàn thành kế hoạch")
