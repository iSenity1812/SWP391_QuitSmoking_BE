...
public class PaymentService {
    ...
    public PaymentOrderResponse createOrder(PaymentOrderRequest request, String clientIP) {
        ...
        try {
            ...
            // DÒNG NÀY SẼ GỌI CONSTRUCTOR KHÔNG THAM SỐ ĐƯỢC TẠO BỞI LOMBOK
            // @NoArgsConstructor
            // Đảm bảo không có constructor thủ công nào khác trong
            // PaymentOrderResponse.java
            PaymentOrderResponse response = new PaymentOrderResponse(); // <--- ĐÂY LÀ DÒNG GÂY LỖI NẾU KHÔNG CÓ
                                                                        // CONSTRUCTOR RỖNG
    ...
    public void confirmVnPayCallback(Map<String, String> fields, String clientIP) {
        ...
        try {
            ...
            throw new PaymentProcessingException("Failed to confirm VNPay payment: " + e.getMessage(), "CALLBACK_ERROR",
                    e);
        }
    }
    ...
    private void processSuccessfulPayment(Transaction transaction, Map<String, String> fields) {
        try {
            ...
            Subscription subscription = subscriptionRepository.findAll().stream()
                    ...
                    .findFirst()
                    .orElseThrow(() -> new PaymentProcessingException("No matching subscription found",
                            "NO_MATCHING_SUBSCRIPTION"));

            MemberSubscription newMemberSubscription = new MemberSubscription();
            newMemberSubscription.setMemberSubscriptionId(UUID.randomUUID());
            ...
            newMemberSubscription.setSubscriptionStatus(SubscriptionStatus.ACTIVE);
            newMemberSubscription.setPurchasedAt(LocalDateTime.now());

            memberSubscriptionRepository.save(newMemberSubscription);
            logger.info("Created new MemberSubscription for member: {} with ID: {}", member.getMemberId(),
                    newMemberSubscription.getMemberSubscriptionId());

            User user = member.getUser();
            if (user != null) {
                com.swp391project.SWP391_QuitSmoking_BE.entity.Role entityPremiumRole = roleRepository
                        .findByRoleName("ROLE_PREMIUM")
                        .orElseThrow(() -> {
