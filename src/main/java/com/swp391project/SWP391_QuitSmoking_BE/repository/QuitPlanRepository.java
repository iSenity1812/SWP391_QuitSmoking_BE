...
public interface QuitPlanRepository extends JpaRepository<QuitPlan, Integer> {
    ...
    List<QuitPlan> findByMemberOrderByCreateDateDesc(Member member);
    ...
    @Query("SELECT q FROM QuitPlan q JOIN FETCH q.member m JOIN FETCH m.user WHERE m.memberId = :memberId ORDER BY q.createDate DESC")
    List<QuitPlan> findByMemberIdWithMemberAndUser(@Param("memberId") UUID memberId);

    Optional<QuitPlan> findFirstByMember_MemberIdAndStatusOrderByCreateDateDesc(UUID memberId,
            QuitPlanStatus quitPlanStatus);
}
