...
public class User implements UserDetails {
    ...
    @Pattern(regexp = "^$|.*\\.(jpg|jpeg|png|gif|webp|bmp|svg)$", message = "Tên file ảnh không hợp lệ hoặc định dạng không được hỗ trợ", flags = Pattern.Flag.CASE_INSENSITIVE)
    @Column(name = "profile_picture", length = 255)
    private String profilePicture;

    @JdbcTypeCode(SqlTypes.JSON) // Annotation của Hibernate để xử lý JSON
    @Column(name = "notification_setting", columnDefinition = "json")
    ...
    public Collection<? extends GrantedAuthority> getAuthorities() {
        ...
        // Ví dụ: Role.SUPER_ADMIN.name() sẽ là "SUPER_ADMIN", sau đó thành
        // "ROLE_SUPER_ADMIN"
