...
@Table(name = "CravingTrackings", uniqueConstraints = {
        @UniqueConstraint(columnNames = { "DailySummaryID", "TrackTime" }) // Đảm bảo unique theo DailySummary và
                                                                           // TrackTime
})
public class CravingTracking {
    ...
    @Column(name = "Situations", columnDefinition = "TEXT")
    private Set<Situation> situations;

    @JdbcTypeCode(SqlTypes.JSON)
    @Column(name = "WithWhoms", columnDefinition = "TEXT")
